...
### Private Synthetics Working

- a "private location" is a collection of "job managers" or ("minions" in legacy)
- the "horde endpoint" is https://synthetics-horde.nr-data.net/ for us based accounts
- the job managers poll the horde endpoint to receive and process jobs
- "ping monitor checks" - a job manager can run about 200 ping monitor checks per minute
- we basically create a "private location" using the ui / api / terraform
- after creating this private location, we get a "key" associated with it
- we use this key to install "job managers"
- 
