...
- resolving conflicts related to concurrent editing - manual approvals in case of conflicts, prs etc work for git, but not in google docs where users expect realtime updates. the conflict resolution needs to be automatic and seamless
...
- my thought - this is possible here unlike in [whatsapp](#whatsapp) because the scale is much less - the number of concurrent editors for e.g. for google doc is capped at 100
- if the software was something like google sheets, we could have locks for the different cells when using collaborative editing, and then we would not have needed the algorithms like [operational transformation](#operational-transformation) and [differential synchronization](#differential-synchronization) described below
...
- unlike [operational transformation](#operational-transformation), it does not have the granularity - not every operation is captured and logged - it is instead performed in batches. so, features like showing the "cursor" is possible with this approach
...
- mention the different deployment strategies like rolling, blue green, canary discussed [here](/posts/high-level-design/#deployment-and-production-testing-patterns)
...
- if a worker somehow fails during the build, just like we discussed in [web crawler](#web-crawler), the message would become visible again in the queue, and another worker can pick it up eventually
- we can also point out how many build metadata / artifacts to retain, how to perform cleanup, etc

![](/assets/img/high-level-design/code-deployment-system-build.png)
...
- now, during the [build phase](#high-level-design---build), we had pushed the build artifact to a blob storage
...
![](/assets/img/high-level-design/code-deployment-system-deploy.png)
...
![](/assets/img/high-level-design/stripe-workflow.png)
...
- modelling them as a state machine makes such invalid transitions impossible
- e.g. find the state diagram of payment here

![](/assets/img/high-level-design/payment-system-state-diagram.png)
...
- so, in our [payment entity](#entities), we do not store the credit card information, but this token instead

![](/assets/img/high-level-design/payment-system-pcidss.png)
...
- solution - "idempotency keys" - every time the customers confirm the payment, they include an idempotency key
- the auth service would store this key in the database
- this way, even if the operation is retried because of for e.g. some network failure, the auth service would know about it
...
- solution - ["transactional outbox pattern"](/posts/high-level-design/#transactional-outbox-pattern). instead of calling the provider as part of the same transaction, commit it to an "outbox table" while updating the database about the payment. this way, a separate process can pick the calls up from the outbox table, and then calls the external provider
...
- finally, we can mention concepts around [message delivery semantics](/posts/high-level-design/#message-delivery-semantics), optimistic concurrency, etc
...
- TODO - https://www.youtube.com/watch?v=8v_Sv3bHNxE
...
![](/assets/img/high-level-design/llm-powered-support-bot.png)
...
- this helps handle "back pressure" - when we get more requests than the server can handle, the requests are queued
