...
- first we typically create the spark session [when running locally](#local-environment-setup-basics). it is already available to us by default when using databricks. the alternative to the builder pattern is creating the **spark config** object manually -
  ```txt
  conf = SparkConf()
