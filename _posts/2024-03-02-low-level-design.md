...
- "director" - has the "algorithm" to help create using builders
...
- subclasses define parts of the algorithm
- without modifying the overall structure of the algorithm
...
- define operations for elements of an object
- without changing the class of this object
...
- if concrete classes increase, we will have to modify all visitors
- so, use the visitor pattern when the element hierarchy is stable but we keep adding new functionality to them
...
class FuelVisitor implements AircraftVisitor {   class FuelVisitor implements AircraftVisitor {
