...
- the actual commit / abort is only performed in the next phase - the commit phase
...
- data pages - 
  ...
  - so for storing one table, the underlying dbms will manage several data pages
- working of offsets -
...
- data blocks - 
  ...
  - a data block is a section in the actual underlying physical memory that can be read from or written to in one single i/o operation
...
- b+ trees - 
  ...
  - recall how binary trees have 1 key and 2 pointers. similarly, m order tree or m ary tree means means a node can have m - 1 keys and m pointers
  ...
  - the value in these nodes is basically the column we index on
  ...
  - then, it can extract the data pages from the data block

![b+ tree](/assets/img/high-level-design/b+-tree.png)

- my understanding - the the b+ trees themselves are stored in "index pages". they hold the values of the column / primary key we are using for indexing for the table. the leaf nodes contain the pointer to the actual data pages
- without all the indexing, the db would -
...
- non clustered indexing - 
  - we have other keys like secondary index, composite index, etc. they use non clustered indexing underneath
...
- "phantom read" -
  ...
  - so, transaction a basically saw different number of rows during different points of the transaction
...
- "partition tolerance" - system continues to operate despite messages being lost over the network
- so, cap theorem states that we can only have two of the three things
- so, we already saw cp and ap, what about ca?
- we can have ca if we have no replicas - only a centralized database i.e. no distributed system
...
- object storage allows "bigger" files compared to dfs, ideal for storing for e.g. database snapshots
...
- drawbacks - not realtime - for instance, we would like logs and metrics to be analyzed realtime so that we can identify and debug production issues quicker
...
- how can a resource server verify the access token provided by the client? - three options - 
  ...
  - both authorization server and resource server can have access to the same shared storage. drawback - a shared storage between two different services
...
- however, we cannot hide the client secret in spa applications, since the entire source code is accessible from the browser, and the client secret would thus be exposed
...
- so, with a mitm attack, if someone gets access to the authorization code, it is not enough. they need the code verifier as well, and they cannot predict it from the code challenge since it is hashed using sha256
