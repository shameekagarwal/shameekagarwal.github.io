...
- **transaction** - a **unit of work** / **atomic** i.e. either one or all operations must succeed. these operations contain dml
- transactions should be small, since they consume a lot of database resources like locks
- if an error occurs midway during a transaction, we need to rollback the changes
...
- when executing a transaction, **database locks** are used to **lock either tables or rows** depending on the database implementation. when something is locked, **other operations will wait on this lock** for it to be released
- initial databases had **2 phase locking**, while modern databases have **mvcc**
- **2 phase locking** - 
  - **readers** use a **shared lock** - shared lock blocks other writers but readers can share the same lock
  - **writers** use an **exclusive lock** - exclusive lock blocks both writers and readers
...
- **dirty read** problem - 
  ...
  - transaction one commits 110 to database
  - transaction two reads 110 from database
  - transaction one tries deducting 10 from second account, but realizes that there are insufficient funds
  - transaction one rolls back the balance by setting balance back to 100
  - so, while the database is at the same state as before transaction one started, transaction two read an inconsistent value committed by transaction one
...
- "authorization code" grant type flow - 
  ...
  - client redirects to authorization server with -
    ...
    - response type - code to specify that it is an authorization code type flow
    - state - help with csrf like attacks
  ...
  - client goes to authorization server with - 
    ...
    - grant type - specify that it is an authorization code grant type flow
    - redirect uri
...
- "implicit" grant type flow (deprecated, removed from oauth 2.1) - 
  ...
  - client redirects to authorization server with -
    ...
    - scope
    - redirect uri - where the authentication server redirects post successful authentication
    - response type - specify that it is an authorization code grant type flow
    - state - help with csrf like attacks
  - resource owners enter their credentials here
  - assume successful authentication here
...
- "password" grant type flow (deprecated, removed from oauth 2.1) - 
  ...
  - client goes to the authorization server with -
    - resource owner's actual credentials
    - client id
  - client gets an access token for the resource owner from the authorization server
...
- "client credentials" grant type flow -
  - client sends request to authorization server with -
    - client id
    - client secret
    - grant type - specify that it is an client credentials grant type flow
    - scope
...
- "refresh token" helps avoiding resource owners from initiating entire login flow again after access token expires
  ...
  - client sends request to authorization server with -
    - client id
    - client secret
    - grant type - specify it is refresh token type flow
    - scope
    - refresh token
...
- my understanding - refresh tokens do not typically have an expiration, but can have one
...
- add the below dependency -
  ...
  ```
- **specify conversion logic for jwt to GrantedAuthority** - 
  ```java
  ...
  jwtAuthenticationConverter.setJwtGrantedAuthoritiesConverter(new JWTToGrantedAuthorityConverter());
  http.oauth2ResourceServer(configurer -> configurer.jwt(
    jwtConfigurer -> jwtConfigurer.jwtAuthenticationConverter(jwtAuthenticationConverter)
  ));
  ```
- recall - specify the path that our application should use to download the public certificate, to help it verify the access and id tokens -
  ```
  spring.security.oauth2.resourceserver.jwt.jwk-set-uri=http://localhost:8080/realms/client-credentials-demo/protocol/openid-connect/certs
  ```
...
- so, now assuming we are doing client credentials flow, we can try getting access token from postman i.e. postman here can act like the client application. it is the `token_endpoint` property in the config url earlier
  ```
  url - http://localhost:8080/realms/spring-6-security/protocol/openid-connect/token
  ...
  client_secret - ...
  ```
- we can see the decoded and parsed version of access token and id token obtained above in the website [jwt.io](https://jwt.io/). it would explain why the convertor maps roles using `realm_access.roles`
- add the token using `Authorization: Bearer <<token>>`
