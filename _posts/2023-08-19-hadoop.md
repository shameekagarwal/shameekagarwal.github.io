...
- and more importantly while doing all of this, hadoop abstracted it away from us
...
- for a mapper to output top n records, it can do so only after all records in the partition it is responsible for have been processed, because mappers are called once per record for all records in the split it is responsible for. so, we use the `cleanup` method for this
...
- e.g. we have a csv where each row represents an order for a customer. so, it contains customer id and total
