...
- we should be able to add or remove servers easily
...
- this communication happens across the cluster using a "gossip based protocol"
...
[read this](/posts/high-level-design/#cdn)

### Functional Requirements

- the cdn should retrieve content from the origin server (push and pull)
- all the peers in a pop ("point of presence") should be able to replicate the changes
- "search" for a user query in the stored content
- delete cached content after ttl

### Non Functional Requirements

- minimize latency
- horizontally scale easily as demand increases
...
- "push cdn" - origin servers push the content to proxy server
- good for static content / less frequently changing content
- using it for frequently changing content can lead to redundant content pushes
- "pull cdn" - the proxy server itself pulls the content from the origin server
...
- this reduces the burden on the origin server, as it does not have to distribute the content to all the proxy servers
...
- multiple layers of cache helps handle this as well - tail content can be kept in for e.g. parent edge servers. the child edge servers can ask parent edge servers for tail, less frequently accessed content, while the child edge servers contain the most frequently used content only, which serves 99% of the use cases
...
- now, the authoritative name server of the cdn is able to factor in the various factors we mentioned above around latency, load, geography, etc to determine the best proxy server to route to, and returns its ip address. i believe this is the "routing system" kicking in. it is basically selecting the best proxy servers for us
...
- "anycast" is used i.e. multiple servers have the same ip address. this way, the request is automatically routed to the nearest server
...
- the third method is using "leases". lease is the duration for which the origin server agrees to notify the proxy server if the data changes. once the lease duration expires, the proxy server requests for a fresh lease
...
- "ixp" or "internet exchange point" - junction between isp, cdns, etc. its a place where different networks connect and exchange traffic
...
### Solution 2 - Logical Clocks

- we can use "logical" or "physical" clocks
- "physical clocks" - of two types - "time of the day" and "monotonic"
- "time of the day clock" - lower resolution compared to monotonic clocks
- "ntp" or network time protocol can move it backwards as well
- "monotonic clocks" can only go forwards
- ntp can only increase or decrease the speed of the clock
- issue - they are not meaningful across two different nodes or even two different processors of the same node
- physical clocks drift with time due to equipment age, manufacturing defect, etc
- so, we instead use "logical clocks"
- it too has two types - "lamport clocks" and "vector clocks"
- one idea in both of these - when events are communicated between nodes, they come with their own "timestamp" as well
- this timestamp is what we are handling using the lamport's algorithm / vector clocks
- "lamports algorithm" - all nodes have a counter
- before executing an event, the node increments its counter
- when a node receives an event, it updates its counter with this value
  ![](/assets/img/high-level-design/lamport-clock-example.png)
- disadvantage - does not help us with "happened before relationship" at global level. my understanding of an example of this
  - imagine 3 events happened at node a
  - then 2 events happened at node b
  - lets say that node a sends its last event to node b
  - now, b will update its counter / see this event's time as 4
  - but this event actually happened even before node b's 1st event
- "vector clocks" - this can capture the happened before relationship accurately
- algorithm - whenever communication happens, the maximum values for each of the node is taken, and then the increment is performed
  ![](/assets/img/high-level-design/vector-clock-example.png)
- cons of this final vector clock approach - the vector clocks can become huge / difficult to store, and hence not fit inside the 64 bit space
...
- "top" - view the list of processes etc
...
- advantage of pull - monitoring system can pull the data without being overloaded by systems sending too much data, as it can pull as per its own schedule
- also, it does not require installing daemons on every server to push these metrics
- "push" - the "distributed data collectors" send their collected data to the "monitoring system" periodically
- advantage of push - when a firewall prevents the monitoring system from accessing the server directly
- also, it is more realtime than the pull strategy
...
- it is divided into client and server like discussed above
...
- "message broker" - we use redis as a message broker for asynchronous communication
- it has a "cluster manager" that helps with automatic failover to secondary shards in case of failures
...
- the multiple requests are processed independently, e.g. one might return an error and another a success, etc

![](/assets/img/high-level-design/redis-pipelining.png)

### Memcached vs Redis

- "simplicity" - memcached management is more difficult compared to redis, redis automates most of it
- "persistence" - memcached uses ram only, while redis provides persistence using aof (append only files) and rdb snapshots as well. memcached uses 3rd party tools for implementing non volatile storage
- "data structure" - in memcached, both key and values are stored as strings. so, values need to be serialized before storing / we cannot manipulate the values. however, redis  supports "data structure storage" instead of retrieving the data, manipulating it and finally storing it back, we can make in house changes. it supports sorted sets, bitmaps, hashmaps, etc
- "multithreading" - memcached can efficiently use multi core system / multithreading unlike redis
- "replication" - again inbuilt feature of redis unlike memcached
