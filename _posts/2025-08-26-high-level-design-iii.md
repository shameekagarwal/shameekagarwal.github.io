...
- note - there are two kinds of state, do not confuse them -
  - "connection state" - for a tcp connection, if packet 1 from client a reaches server x, packet 2 from client a should reach server x as well
  - "application / session state" - auth information, shopping cart, etc
- my understanding - the below conversation of stateful vs stateless is talking about the connection state
- "stateful" vs "stateless" -
  - we are running multiple instances of the load balancer
  - however, as pointed we should ensure that packets from a client reach the same backend instance
  - "stateful load balancers" - using "gossip protocol" etc, the load balancers sync their state with each other
  - this way, even if the packets reach a different load balancer instances, they would know which backend server to route the traffic to
  - hence there is maintenance overhead to keep this state in sync
  - "stateless load balancers" - it would use consistent hashing to route to the random backend server
  - the hash is performed using these 5 params - src ip and port, dest ip and port, protocol
  - recall how plain hashing breaks when we add or remove backend servers, which is why we use consistent hashing. instead of all, only the 1/n servers that require rebalancing would break
  - stateless load balancers can continue maintaining some kind of "local state" i.e. it need not be synced across instances. this helps handle "in flight" connections in case servers are added or removed
  - so, the stateless logic has little maintenance, but if a backend server is added and the packet reaches a different load balancer instance, our flow breaks
- "layer 4" vs "layer 7" -
  - "layer 4" - load balancing is performed using tcp / udp
  - "layer 7" - based on application layer protocols
  - layer 4 forwards the tcp / udp communication from a client to the same backend server
  - layer 4 is faster because it need not inspect the payload
...
- till now, we discussed primary secondary replication models
- now, we discuss "peer to peer / leaderless replication" - 
...
- now, what if we wanted to retrieve documents using "secondary" instead of "primary" index? assume customer name is our secondary index. we can use one of the two techniques below - 
  ...
  - "partition secondary indices by term" - we manage the secondary index information separately on separate servers. this is also called "global secondary index". now, this index can be too big to manage in one server as well, and it too can be split into multiple shards. of course, sharding this index table would be done using name in this case - a-m are in partition 0, and names n-z in partition 1. so, for john, we go to partition 0. now, these index table partitions will map names to customer ids - john -> [123, 789]. now, we only need to query the data shards that contain the ids 123 and 789
...
- no sql databases can store so much data because of lsm trees
- stands for "log structure merge" tree
- optimized for fast writes
- relational databases use "b trees"
- b trees unlike lsm trees are optimized for fast reads
- they involve "random io" i.e. updating multiple pages of disk
- in an lsm tree - the writes are stored in an in memory table called "mem table"
- once it reaches a certain size, it is flushed to disk as an "ss table" (sorted string table)
- as more data comes in, more of these ss tables are generated
- these ss tables are immutable - which is why updates are stored in the new ss tables, same for deletes i.e. they are stored as delete markers or "tombstone records" (refer diagram)
- to serve reads, go to the latest ss table, then the second latest ss table and so on, till the key is found
- searching inside an ss table is fast, as each ss table should be sorted
- to improve reads, there is a periodic "compaction" process
- since ss tables are already sorted, it can be similar to merge sort algorithm
- these ss tables also contain "bloom filters" to avoid redundant lookups

![](/assets/img/high-level-design/lsm-trees.png)
...
- mention points like injecting failures using chaos monkey, canary deployments for new features, etc
...
- "hit ratio" like metrics are used to monitor cdns
- "key" - url, parameters, etc
- point out that we should remove "utm" (i.e. analytics) related parameters during cdn lookups
- "request coalescing" - assume we hit the ttl for a hot key. this means all requests hit and bombard the origin servers with the same request. solution - "request coalescing". if our system sees that a request has already been sent for a specific key to the origin servers, rest of the requests for the same key are made to wait. once we have the result, all the requests are returned these results
- start fetching asynchronously when we are nearing the ttl, not when we hit it
- we can serve outdated content if the origin servers are down, depending on consistency requirements
...
- consistency < availability example - we would rather have the rate limiter be up all the time and use outdated rules, rather than have it down to always use the most upto date rules
...
- when we reject a request, along with the 429 status code, we can also send these standard headers -
...
- implementation (pseudo logic) - we use "lazy refill" - instead of refilling the bucket at every interval, when we get a request, we first check time elapsed since last refill
...
- so, if the bucket "overflows", we reject the incoming requests. this is the rate limiting part
...
- we can deduplicate data using hash - useful for popular content, images, etc. this helps save on storage
...
- we use the following algorithms on top of inverted index -
  ...
  - "relevance" - helps measure the accuracy between "search query" and "search results". talk about [tf-idf for this](/posts/hadoop/) here
...
- note - revise concepts from [elasticsearch](/posts/elasticsearch/) as well
...
- recall that this is similar to the "scatter gather" pattern
...
- the log accumulator then batches these logs and pushes them to the "pubsub system"
- remember - applications write logs to files, while the log accumulators tail these and forward to the pub sub system. this helps make the logging "asynchronous" - logging should not happen in the client's critical user journey. it also decouples the application from temporary failures in the log accumulator
...
- recall - sampling can be "head based" or "tail based", so maybe head sampling can be implemented at log accumulator (e.g. remove health check logs), while tail based sampling at the filterer (e.g. remove logs for entire requests e.g. search results displayed on home screen)
...
- "scalability" - execute a huge number of tasks per day
...
- most tasks are short lived (seconds to few minutes). long running tasks should have features like periodic checkpointing implemented in the application itself for failure recovery
...
- point out long running tasks, heavy tasks, etc during requirement gathering to show depth
...
- for queueing, a simple first come first serve may not be enough. so, we categorize tasks -
  - urgent tasks
  - non urgent tasks
  - periodic tasks
...
- it can be determined using the number of followers someone has, the type of post (public or private), etc
...
- my doubt cleared - how does a sharded counter ensure that a user does not like a post twice - it does not 😛. we use the usual flow of inserting a row for the user_id, tweet_id. inserting a row can work fine unlike updating a row that contains say like_count, as updating would require locks. this is why to reduce that contention etc, we use sharded counters, while the dedupe etc can continue happening on the relational database
- also, sharded counters help us [display trends](#displaying-trends-for-a-user)
