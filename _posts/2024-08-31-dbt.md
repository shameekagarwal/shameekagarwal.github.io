...
- **environment type** can be **development** or **deployment** - we select deployment here, since development is already there
- then, we select a deployment type - typically **production**
- just like we saw in [getting started](#getting-started), we need to set the snowflake account url, database, warehouse, etc
