...
- a technique for breaking down large dbt projects into multiple projects. some advantages - 
  - helps collaborate efficiently - e.g. avoid conflicts due to analytics engineers working on entirely different features
...
- using model contracts, we can ensure - column names, data type of each column, etc
  ...
  - finally, we need to list the constraints like not null
      ...
      columns:
        - name: number_in_trilogy
          ...
          constraints:
            - type: not_null
  ```
- difference between [tests](#tests) and [contracts](#model-contracts) -  
  - contracts check shape of data, while test check content of data
  - constraints are like a pre-flight check - bad data is not even allowed to enter the system (i.e. `dbt run` itself would fail). tests are like a post-hoc check - bad data can get into our system and tests notify us about it
- note - this is specific to the data warehouse we use, e.g. different data warehouses will support different data types
...
- if downstream users do not specify a version i.e. they use `(( ref('fct_orders') ))`, the latest version will be used by default. to change this behaviors, we can configure what version should be used by default using `latest_version`
...
- **public models** - can be accessed by models in any group or project. helps with multi project collaboration. e.g. staging models
...
- first, ensure that models in the upstream project are [**public**](#model-group-and-model-access-modifiers)
- also, there should be at least one successful run in the production environment before we can start importing across projects
