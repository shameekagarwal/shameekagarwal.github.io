...
- messages in a partition get an id called offsets. note - so offsets are specific to a partition
...
- kafka is append only i.e. we can just add data to it
- immutability - once data is written into a partition, it cannot be updated / deleted
- we interact with kafka producers and consumers in our code, and it internally handles batching etc
...
- consumers in kafka use a pull model i.e. consumers request for data from the brokers, instead of the brokers pushing data into the consumers
...
- retries - this is producer retries not consumer, don't confuse with concepts like dlq here 😅. retries here refer to transient failures like kafka saves the message but acks fail, required number of brokers (min insync replicas) are unavailable at the time so kafka cannot save the message, etc. focussing on the newer versions here -
  ...
  - delivery timeout (`delivery.timeout.ms`) is set to 120000 i.e. 2 minutes by default. now retries is infinite does not mean producer would just keep retrying endlessly in case of failure, since the time it first sent the message, it would keep retrying until this timeout occurs. after this, an exception is thrown and the failed batch is removed from the buffer of the producer
...
- `partitioner.class` - in earlier versions of kafka, if we specify no key for our message, the messages are sent to partitions in round robin fashion using round robin partitioner. disadvantage - for e.g. remember batching happens at partition level, so this means we cannot utilize batching effectively, since there is a batch being created for every record. sticky partitioner is the default in newer versions of kafka. this means that instead of round robbin, producer would fill one batch (until `linger.ms` or `batch.size`) and then send to one partition. after this, a new batch is started. so we can leave this property untouched in newer versions
...
- sync will wait for the acknowledgement from the broker while async will not. this means with sync, we poll for the next batch only after the offsets for the current batch is saved to kafka. it will automatically retry as well if the acknowledgement is not received. unlike sync, async is fast. however with async, we can run into a scenario where offsets of older batches overwrite offsets of newer batches. thus, we end up reprocessing new batches in case of restarts
...
- consumers send a heartbeat every `heartbeat.interval.ms` seconds (3 seconds by default), and if no heartbeats are received for `session.timeout.ms` seconds (45 seconds by default), the consumer is considered dead. this heartbeat related functionality is carried out by the dedicated heartbeat thread
- if a new poll call is not made in `max.poll.interval.ms`, the consumer is considered to have failed processing of that batch, and kicked out of the consumer group and a rebalance for that partition is triggered
- for replicating data across kafka clusters, e.g. if cluster is across regions, or for e.g. when we are hitting performance limits with one kafka cluster and need multiple kafka clusters, we can use tools like mirror maker 2. replication can be active active (two way replication, e.g. data producers in multiple regions) or active passive (one way, e.g. for global resiliency)
...
- kafka has data retention for 7 days by default (default value of `log.retention.hours`)
...
- so, there is one (the latest) active segment which kafka writes to, and other segments are readonly
...
- a segment is closed and a new one is started when either the `log.segment.bytes` size is reached or `log.roll.ms` time is reached
- similarly, segments are deleted when size of all segments reaches `log.retention.bytes` or if the age of a segment crosses `log.retention.hours`
- kafka does not delete from topics message by message, and it does not have to for e.g. lock the topic when the data starts growing. it simply cleans up the old segments, as the writes only happen on the latest segment
...
- queue - the buffer that stores messages before the consumers consume from this queue
...
- to ensure message durability, we need to set durability at all levels -
  - mark exchanges and queues as durable - we need to set this when creating them
...
- rabbitmq also has two types of queues -
  - "classic" - cannot withstand node failure, since it is only present on the primary node
  - "quorum" - replicated across different servers. maintains consistency using quorum
- remember that durability and quorum affect performance
...
- "fan out exchange" - helps implement the publish subscribe pattern by broadcasting the message to all queues
  ...
  - notice how we specify the exchange name when sending the message, and since the routing key is ignored, we do not set it at all
    ```txt
...
- "topic exchange" - 
  - with direct exchange, we can only route messages using a single criteria
...
- dead letter exchanges - 
  ...
  - note - we can change the routing key of the message when moving a message to the dead letter exchange
  - we cannot throw any exception for this to work - we need to throw `AmqpRejectAndDontRequeueException`
- time to live - 
  - if a message is present in a queue for longer than this timeout, it is declared "dead"
  - the message from the actual queue would be moved into the dead letter exchange if configured
  - so, we can automatically move messages to a dead letter exchange in both scenarios - timeouts and errors
- retry mechanism - 
